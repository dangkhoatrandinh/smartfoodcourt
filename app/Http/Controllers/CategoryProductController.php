...
class CategoryProductController extends Controller
{
    ...
    public function viewcategorybyid($category_product_id) {
        ...
        $product_data = DB::table('tbl_product')->join('tbl_category_product','tbl_product.category_id','=','tbl_category_product.category_id')->join('tbl_brand','tbl_product.brand_id','=','tbl_brand.brand_id')->where('product_status','1')->where('brand_status','1')->where('category_status','1')->orderby('product_id')->where('tbl_product.category_id',$category_product_id)->get();
        $cate_name = DB::table('tbl_category_product')->where('category_id',$category_product_id)->limit(1)->get();
        return view('pages.categorybyid')->with('cate_product',$cate_product)->with('brand_product',$brand_product)->with('product_data',$product_data)->with('cate_name',$cate_name);
    }
}
